app.controller(
  "testCtrl",
  function ($scope, $http, $routeParams, $rootScope, $interval) {
    $rootScope.hide = 1;
    $rootScope.timer = 600;
    $rootScope.KQ = [];
    $rootScope.checkKq = [];
    ...
    $http
      .get("db/Quizs/" + $routeParams.id + ".json")
    ...
    function checkAnswer() {
      ...
      if (
        $scope.questions[$scope.indexQues].AnswerId ==
        $scope.elem[$scope.indexQues].answer
      ) {
        Swal.fire({
          icon: "success",
          ...
          text:
            "Bạn được cộng " +
            $scope.questions[$scope.indexQues].Marks +
            " điểm",
        ...
        mark++;
        $rootScope.Student.marks = angular.copy(
          $scope.questions[$scope.indexQues].Marks +
            parseInt($rootScope.Student.marks)
        );
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          Swal.fire({
            ...
            html:
              '<h2 style="font-weigh:bold;color:red">Điểm của bạn là: ' +
              mark +
              "</h2>" +
              "<h3>Bài thi sẽ kết thúc sau " +
              $rootScope.timer +
              " giây</h3>",
            icon: "success",
            showConfirmButton: false,
    ...
    var stop = $interval(function () {
      ...
      if ($rootScope.timer > 0) {
        ...
        if ($rootScope.Student != null) {
          firebase
            .database()
            .ref("Students/" + $rootScope.Student.username)
            .set($rootScope.Student);
        }
